...
class Game{
    ...
    public void play(){
        ...
        if(winner != null){
            System.out.println("Thw winner is " + winner.getName());
        }
    ...
    private void takeTurn(Player player, int consecutiveSixes, int turnStartPosition){
        ...
        if(nextPosition > board.getSize()){
            System.out.printf(
                "OOps, %s needs to land exactly on %d. Turn skkiped.%n", 
        ...
        if(finalPosition > nextPosition) {
            System.out.printf(
                "Wow! %s found a ladder at %d and climbing to %d.%n",
        ...
        if (roll == 6){
            System.out.printf(
                "%s rolled a 6 and gets another turn!%n", player.getName());
        ...
        public Game build(){
            if(board == null || players == null || dice == null){
                throw new IllegalArgumentException(
                  "Boards, Players and Dice must be set"  
                );
            }
